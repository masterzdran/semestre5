...
{
    HWND hDlg = (HWND)p;
	int number = 1;//number_Next_Plane_Land++;
	wchar_t name[3] = {'A','0','1'};
	//criar aviao
	//Plane* plane = new Plane();
	int idPista = 1;
	//valor retornado pelo esperaPistaparaAterrar!!
	idPista = gestor->esperarPistaParaAterrar();
...
DWORD WINAPI thAviaoDescolar(LPVOID p) 
{
	...
	//PNamePlane pl = (PNamePlane)p;
	//HWND hDlg = pl->handle;
	//int number = pl->number;

	//criar aviao
	//Plane* plane = new Plane();
	int idPista = 0;
	//valor retornado pelo esperaPistaparaAterrar!!
	//idPista = gestor->esperarPistaParaDescolar();
