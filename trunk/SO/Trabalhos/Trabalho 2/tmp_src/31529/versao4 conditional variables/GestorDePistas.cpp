...
class GestorDePistas : public IGestorDePistas
{
	...
	CONDITION_VARIABLE _cvWaitingListLanding;
	CONDITION_VARIABLE _cvWaitingListLiftoff;
	...
	Plane * UseLaneTo(Plane::PlaneDirection direction, Plane * planeList,
		CRITICAL_SECTION csPlaneList, PCONDITION_VARIABLE cvWaitList)
	...
	{
		//EnterCriticalSection(&_csPlaneListToLand);
		//EnterCriticalSection(&_csPlaneListToLift);
		if((_bFuracao || pd==Plane::LAND) && _planeListToLand->next != _planeListToLand
			|| pd==Plane::LIFTOFF && _planeListToLift->next == _planeListToLift)
		{
			WakeConditionVariable(&_cvWaitingListLanding);
		...
		{
			WakeConditionVariable(&_cvWaitingListLiftoff);
			//ReleaseSemaphore(_sWaitingListLiftoff,1,NULL);
		}
		//LeaveCriticalSection(&_csPlaneListToLift);
		//LeaveCriticalSection(&_csPlaneListToLand);
	}
...
public:
	...
	GestorDePistas(int nLanes)
	{
		...
		InitializeConditionVariable(&_cvWaitingListLanding);
		InitializeConditionVariable(&_cvWaitingListLiftoff);
	...
	virtual Plane * esperarPistaParaAterrar()
	{
		...
		Plane * p = UseLaneTo(Plane::LAND,_planeListToLand,_csPlaneListToLand,&_cvWaitingListLanding);
	...
	virtual Plane * esperarPistaParaDescolar()
	{
		...
		Plane * p = UseLaneTo(Plane::LIFTOFF,_planeListToLift,_csPlaneListToLift,&_cvWaitingListLiftoff);
